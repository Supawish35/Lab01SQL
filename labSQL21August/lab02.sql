...
SELECT c.CompanyName [Customer Name],
...
WHERE MONTH(o.OrderDate) IN (1, 2, 3) AND YEAR(o.OrderDate) = 1997
...
SELECT distinct e.FirstName + ' ' + e.LastName AS [FullName], e.Title
FROM Employees e
    JOIN Orders o ON e.EmployeeID = o.EmployeeID
    JOIN Customers c ON o.CustomerID = c.CustomerID
...
SELECT o.OrderID,
       ...
       o.RequiredDate,
       ROUND(SUM(od.Quantity * od.UnitPrice * (1 - od.Discount)), 2) AS TotalAmountDue
FROM Orders o
    JOIN Employees e ON o.EmployeeID = e.EmployeeID
    JOIN Customers c ON o.CustomerID = c.CustomerID
    JOIN [Order Details] od ON o.OrderID = od.OrderID
